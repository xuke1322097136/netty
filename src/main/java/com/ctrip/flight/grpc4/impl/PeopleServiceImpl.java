...
/**
 * Created by xuke
 * Description: 这里测试的事第四种情况：客户端发送的是流式数据，服务器端返回的也是流式数据
 * Date: 2019-09-24
 * Time: 1:05
 */
public class PeopleServiceImpl extends PeopleServiceGrpc.PeopleServiceImplBase {
    ...
    public void getPeoplesByAge(PeopleRequest request, StreamObserver<PeopleResponse> responseObserver) {
        ...
        // 为了消除波浪线，在这把名字改了一下。
        responseObserver.onNext(PeopleResponse.newBuilder().setName("aaaa").setAge(18).setCity("北京").build());
        responseObserver.onNext(PeopleResponse.newBuilder().setName("bbbb").setAge(19).setCity("上海 ").build());
        responseObserver.onNext(PeopleResponse.newBuilder().setName("cccc").setAge(20).setCity("广州").build());
        responseObserver.onNext(PeopleResponse.newBuilder().setName("dddd").setAge(21).setCity("深圳").build());
    ...
    public StreamObserver<StreamRequest> biTalk(StreamObserver<StreamResponse> responseObserver) {
        return new StreamObserver<StreamRequest>() {
            ...
            @Override
            public void onCompleted() {
                // 这里表示的是客户端把连接关闭了，此时服务器端也得把连接关闭了。不是跟客户端一样打印输出。
                // 搞清楚客户端和服务器端之间的逻辑关系
             responseObserver.onCompleted();
            }
        };
